...
describe("Authentication", () => {
  test("User is able to sign up only once", async () => {
    const username = `user-${Math.random()}@example.com`;
    const password = "12345678";

    console.log("Attempting first signup...");
    ...
    console.log("First Signup Response:", response?.data || response);
    ...
    console.log(
      "Duplicate Signup Response:",
      updatedResponse?.data || updatedResponse
    );
    expect(updatedResponse.status).toBe(400);
    expect(updatedResponse.data.message).toBe("User already exists");
  });

  test("Signup request fails if the username is empty", async () => {
    const invalidUsername = "not-an-email";
    const password = "123456789";

    console.log("Attempting signup with invalid username...");
    const response = await axios.post(`${BACKEND_URL}/api/v1/signup`, {
    ...
    console.log("Invalid Username Response:", response?.data || response);
    expect(response.status).toBe(400);
    expect(response.data.message).toBe("Validation failed");
    expect(response.data.errors[0].message).toContain("Invalid email address");
  });
  ...
  test("Signin succeeds if the username and password are correct", async () => {
    ...
    console.log("Creating user for signin...");
    ...
    console.log("Signup Response:", signupResponse?.data || signupResponse);
  ...
  test("Signin fails if the username does not exist", async () => {
    const username = "nonexistent@example.com";
    const password = "123456789";

    console.log("Attempting signin with non-existent username...");
    const response = await axios.post(`${BACKEND_URL}/api/v1/signin`, {
      username,
      password,
    });
    ...
    console.log(
      "Non-existent Username Signin Response:",
  ...
  test("Signin fails if the password is incorrect", async () => {
    ...
    console.log("Creating user for incorrect password test...");
    const signupResponse = await axios.post(`${BACKEND_URL}/api/v1/signup`, {
      username,
    ...
    console.log("Attempting signin with wrong password...");
    ...
    console.log(
      "Incorrect Password Signin Response:",
      signinResponse?.data || signinResponse
    );
    expect(signinResponse.status).toBe(403);
    expect(signinResponse.data.message).toBe("Invalid password");
  });
});
